...
import { DocumentDuplicateIcon, CheckIcon, PencilIcon } from '@heroicons/react/24/outline';
import { useAuth } from '@/hooks/useAuth';

const serviceAddress = process.env.NEXT_PUBLIC_SERVICE_ADDRESS || '0x000000000000000000000000000000000000dEaD';

const ADJECTIVES = ['Happy', 'Lucky', 'Clever', 'Bright', 'Swift', 'Witty', 'Brave', 'Kind'];
const NOUNS = ['Bee', 'Buzz', 'Honey', 'Hive', 'Worker', 'Queen', 'Garden', 'Flower'];
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch transactions
        const transactionsResponse = await fetch(`/api/user/${params.address}/transactions`);
        const transactionsData = await transactionsResponse.json();
        setTransactions(transactionsData.transactions ?? []);

        // Fetch withdrawals
        const withdrawalsResponse = await fetch(`/api/user/${params.address}/withdrawals`);
  ...
  const handleUpdateNickname = async () => {
    try {
      const response = await fetch(`/api/user/${user?.uid}/update-username`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: nickname || generateRandomNickname() }),
      });
      ...
      setError(error instanceof Error ? error.message : 'Failed to update nickname');
    }
  };
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl shadow-xl p-8">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-4xl font-bold text-white mt-2">{(profile.balance || 0.00).toFixed(2)} BUZZ</p>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p className="text-lg font-medium text-gray-900">{profile.username || 'Not set'}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Email</p>
            <p className="text-lg font-medium text-gray-900">{profile.email || 'No email'}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Total Earned</p>
            <p className="text-lg font-medium text-gray-900">{profile.totalEarned} BUZZ</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Reputation</p>
            <p className="text-lg font-medium text-gray-900">{profile.reputation} points</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        ...
        <div className="space-y-6">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr>
                    ...
                    <th className="px-4 py-3 bg-gray-50 text-right text-xs font-medium text-gray-500 uppercase">Amount</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {transactions.map((tx) => (
                    <tr key={tx.id}>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900 text-right">{tx.amount} BUZZ</td>
          ...
          {withdrawals?.length > 0 && (
            <div className="mt-8">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Withdrawals</h3>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead>
                    <tr>
                      <th className="px-4 py-3 bg-gray-50 text-right text-xs font-medium text-gray-500 uppercase">Amount</th>
                      <th className="px-4 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">Address</th>
                      <th className="px-4 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">Status</th>
                      <th className="px-4 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">Date</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {withdrawals.map((withdrawal) => (
                      <tr key={withdrawal.id}>
                        <td className="px-4 py-3 text-sm text-gray-900 text-right">{withdrawal.amount} BUZZ</td>
