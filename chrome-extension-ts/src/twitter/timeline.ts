...
import { extractAuthorInfo, convertTwitterPostToMarkdown } from './common';
  ...
  try {
    ...
    tweets.forEach((tweet, index) => {
      ...
      if (repostElement) {
        ...
        const quoteComment = hasQuote ? (contentElement.textContent || '') : undefined;
...
export async function extractTwitterPostsFromAi(html: string, settings: Settings): Promise<TwitterPost[]> {
  try {
